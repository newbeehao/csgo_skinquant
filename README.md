...
**为什么选 Go 而不是 Python 做 Multi-Agent?**
Go 的 goroutine + channel 让 DAG 调度器自然优雅,同样的编排逻辑用 asyncio 需要多一层心智负担。对于"多 Agent 独立调 API 后汇总"这种典型场景,Go 的并发模型是降维打击。

**为什么工具失败时返回字符串而非 error?**
让 LLM 看到"工具报错了"然后自主决策 (换参数重试 / 调用别的工具 / 告诉用户查不到),这才是 Agent 行为。直接中断是脚本行为。

**为什么把手续费计算封装成工具而不让 LLM 算?**
LLM 算多步乘除法不稳定,而手续费是确定性计算。"LLM 负责语言,代码负责数字"是 Agent 工程的黄金分工。

**为什么 system prompt 里要注入当前日期?**
LLM 训练数据有截止日期,它理解的"最近"是训练时的"最近"。在 prompt 里显式注入 `time.Now()`,Agent 才能正确理解"最近 30 天的新闻"等时效性查询。这是所有生产级 Agent 的标准做法。

**为什么用 WaitGroup 而不是 errgroup?**
我们希望即使某个 Agent 失败,其他人的结果仍然保留(符合真实产品的容错需求)。`errgroup.WithContext` 的"一人失败取消所有"语义在这里反而是伤害。
